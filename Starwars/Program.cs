...
namespace Starwars
{
    class Program
    {
        public static void WriteListToConsole(IEnumerable items)
        {
            foreach (Planet o in items)
            {
        ...
        static void Main(string[] args)
        {
            ...
            //Opgave 1
            var Opgave1 = planets.Where(s => s.Name.StartsWith("M"));

            //Opgave 2
            var Opgave2 = planets.Where(s => s.Name.ToLower().Contains("y"));

            //Opgave 3
            var Opgave3 = planets.Where(s => s.Name.Length > 9 && s.Name.Length < 15);

            //Opgave 4
            var Opgave4 = planets.Where(s => s.Name.Substring(1, 1) == "a" && s.Name.Last().ToString() == "e");

            //Opgave 5
            var Opgave5 = planets.Where(s => s.RotationPeriod > 40).OrderBy(s => s.RotationPeriod);

            //Opgave 6
            var Opgave6 = planets.Where(s => s.RotationPeriod > 10 && s.RotationPeriod < 20).OrderBy(s => s.Name);

            //Opgave 7
            var Opgave7 = planets.Where(s => s.RotationPeriod > 30).OrderBy(s => s.Name);

            //Opgave 8
            var Opgave8 = planets.Where(s => s.RotationPeriod > 30 || s.SurfaceWater < 50).Where(s => s.Name.ToLower().Contains("ba")).OrderBy(s => s.Name);

            //Opgave 9
            var Opgave9 = planets.Where(s => s.SurfaceWater > 0).OrderByDescending(s => s.SurfaceWater);

            //Opgave 10
            float pi = 3.1415926535f;
            var Opgave10 = planets.Where(s => s.Diameter > 0 && s.Population > 0).OrderBy(s => (4 * pi * (s.Diameter * s.Diameter)) / s.Population);

            //Opgave 11
            var Opgave11Except = planets.Where(s => s.RotationPeriod > 0);
            var Opgave11 = planets.Except(Opgave11Except);

            //Opgave 12 
            var Opgave12FirstAOrLastS = planets.Where(s => s.Name.Substring(0, 1) == "A" || s.Name.Last().ToString() == "s");
            var Opgave12Rainforest = planets.Where(x => x.Terrain != null).Where(x => x.Terrain.Contains("rainforests"));
            var Opgave12 = Opgave12FirstAOrLastS.Union(Opgave12Rainforest);

            //Opgave 13 
            var Opgave13 = planets.Where(x => x.Terrain != null).Where(x => x.Terrain.Contains("deserts") || x.Terrain.Contains("rocky deserts") || x.Terrain.Contains("desert"));


            //Opgave 14 
            var Opgave14 = planets.Where(x => x.Terrain != null).Where(x => x.Terrain.Contains("swamps") || x.Terrain.Contains("swamp")).OrderBy(x => x.RotationPeriod).ThenBy(x => x.Name);

            //Opgave 15
            var Opgave15 = (from f in planets
                where Regex.IsMatch(f.Name, @"([aeiou])\1")
                select f); ;

            //Opgave 16
            var Opgave16 = (from f in planets
                where Regex.IsMatch(f.Name, @"([klrn])\1")
                select f).OrderByDescending(f => f.Name);

            WriteListToConsole(Opgave16);
            Console.ReadKey();
        }
